...
async function publishAction(opts: { force?: boolean }) {
      ...
      if (result.createdPr) {
        ...
        if (openPr) {
          try {
            ...
            console.log(`Couldn't open a browser (${(e as Error).message}). Use the link above.`);
          }
        }
      }
 ...
 */
async function maybeAutoFixManifest(dir: string): Promise<"clean" | "unfixable" | "wrote-fix" | "aborted"> {
