 ...
 * @param opts.command - Command name for re-run hints (e.g. "ebr include").
 * @returns true if the error was recognized and rendered; false if the
 *   caller should rethrow. The caller sets process.exitCode on a true result.
 */
export function renderCliError(err: unknown, { command }: { command?: string } = {}): boolean {
