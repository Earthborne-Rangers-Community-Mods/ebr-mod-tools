...
import type { ProgressEvent, ModValues, Manifest } from "core/types.js";
...
class NewModForm {
  ...
  selectedMaps = $state<string[]>([]);
  selectedSets = $state<string[]>([]);

  // Where the mod folder will be created (its parent directory).
  parentDir = $state<string | null>(null);
   ...
   */
  async #stampAndInclude(dir: string, manifest: Manifest) {
    ...
    for (const branch of this.scaffoldsToStamp) {
      try {
        ...
        this.warnings = [...this.warnings, { kind: "scaffold", ref: branch, detail: (err as Error)?.message ?? "" }];
      }
    }
    ...
    for (let i = 0; i < campaignsToInclude.length; i++) {
      ...
      try {
        ...
        this.warnings = [...this.warnings, { kind: "campaign", ref: id, detail: (err as Error)?.message ?? "" }];
