...
class SetupStore {
  ...
  detectedLogin = $state<string | null>(null);
  ...
  forks = $state<{ baseContent: string | null; registry: string | null }>({ baseContent: null, registry: null });
  /** Reachability of each fork after an active status check (null = unchecked). */
  baseForkReachable = $state<boolean | null>(null);
  registryForkReachable = $state<boolean | null>(null);
  ...
  manualForks = $state<Array<{ repo: string; browserUrl: string }>>([]);
  #initialized = false;
  ...
  async runSetup() {
    await runGuarded(
      ...
      async () => {
        ...
        const manual: Array<{ repo: string; browserUrl: string }> = [];
        for (const repo of [BASE_CONTENT_REPO, REGISTRY_REPO]) {
          ...
          if (result.status === "manual") {
            manual.push({ repo, browserUrl: (BROWSER_FORK_URLS as Record<string, string>)[repo] });
          }
        }
