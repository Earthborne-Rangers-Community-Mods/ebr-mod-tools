 ...
 *
 * @param params.dir - Directory containing the existing clone.
 * @param params.manifest - Complete manifest object to write.
 * @param callbacks.onProgress - Progress callback ({ step, message }).
 */
export async function scaffoldModIntoClone({ dir, manifest }: { dir: string; manifest: Manifest }, { onProgress }: ProgressOptions = {}): Promise<{ modDir: string; manifest: Manifest; branch: string }> {
 ...
 *
 * @param params.dir - Directory to scaffold into (must be empty or not exist).
 * @param params.manifest - Complete manifest object to write.
 * @param params.forkUrl - HTTPS URL of the user's fork (e.g. "https://github.com/user/ebr-mod-base-content").
 * @param params.baseRepoUrl - Override the upstream base-content URL (tests).
 * @param callbacks.onProgress - Progress callback ({ step, message }).
 * @throws {ManifestError} If directory contains unexpected files.
 */
export async function scaffoldMod({ dir, manifest, forkUrl, baseRepoUrl = BASE_REPO_URL }: { dir: string; manifest: Manifest; forkUrl: string; baseRepoUrl?: string }, { onProgress }: ProgressOptions = {}): Promise<{ modDir: string; manifest: Manifest; branch: string }> {
  const modBranch = getModBranchName(manifest.id);
 ...
 */
function buildPrBody(manifest: Manifest, commitHash: string, isUpdate: boolean): string {
  const lines = [
 ...
 * @param options.base - Target branch (e.g., "main").
 * @param options.head - Source ref (e.g., "user:publish/mod-id").
 * @param options.title - Pre-filled PR title.
 * @param options.body - Pre-filled PR body.
 */
function buildCompareUrl({ registryOwner, registryRepo, base, head, title, body }: { registryOwner: string; registryRepo: string; base: string; head: string; title: string; body: string }): string {
 ...
 *
 * @param options.workerUrl - POST /create-pr endpoint.
 */
async function requestPrViaWorker({ workerUrl, forkOwner, branch, title, body, fetchImpl = fetch }: { workerUrl: string; forkOwner: string; branch: string; title: string; body: string; fetchImpl?: typeof fetch }, onProgress?: ProgressCallback): Promise<PrResult | null> {
  try {
    ...
    onProgress?.({ step: "create-pr-failed", message: `Auto-PR failed (${(err as Error).message}); falling back to compare URL.` });
    return null;
  }
}
 ...
 * @param forkUrl - e.g. "https://github.com/user/ebr-mod-registry"
 */
export function forkOwnerFromUrl(forkUrl: string): string | null {
 ...
 */
async function fetchPublishedModEntry({ registryOwner, registryRepo, modId, fetchImpl = fetch }: { registryOwner: string; registryRepo: string; modId: string; fetchImpl?: typeof fetch }): Promise<RegistryEntry | null> {
  const url = `${RAW_CONTENT_BASE}/${registryOwner}/${registryRepo}/${REGISTRY_BASE_BRANCH}/${MODS_DIR}/${modId}.json`;
  let res;
  try {
    res = await fetchImpl(url);
  } catch (err) {
    throw new GithubError("publish", `Could not reach the registry: ${(err as Error).message}`);
  }
 ...
 */
async function discardAndClone(cloneDir: string, forkCloneUrl: string, { onProgress }: ProgressOptions = {}) {
 ...
 */
async function prepareCloneForPublish(cloneDir: string, forkCloneUrl: string, branchName: string, { onProgress }: ProgressOptions = {}) {
 ...
 * credentials.
 *
 * @param params.cloneDir - Local working clone directory.
 * @param params.registryForkUrl - HTTPS URL of the user's registry fork.
  ...
  if (reusable) {
    try {
      ...
      try {
        ...
        const e = reclErr as { cause?: unknown };
 ...
 *
 * @param options.dir - Mod directory containing ebr-mod.json.
 * @param options.registryForkUrl - HTTPS URL of the user's ebr-mod-registry fork.
 * @param options.force - Skip the unpushed-changes check.
...
export async function publishMod(
  ...
  let includedModWarnings: IncludedModWarning[] = [];
  ...
  let createdPr: PrResult | null = null;
 ...
 */
export function forkUrlFor(login: string, repo: string): string {
 ...
 */
export async function resolveCredentialLogin({ runImpl, interactive = false, getUserImpl = getAuthenticatedUser, borrowTokenImpl = borrowCredentialToken }: { runImpl?: Function; interactive?: boolean; getUserImpl?: (token: string) => Promise<{ login: string }>; borrowTokenImpl?: (opts?: object) => Promise<string | null> } = {}): Promise<string | null> {
 ...
 * required.
 *
 * @param params.owner - Upstream repo owner.
 * @param params.repo - Upstream repo name.
 * @param params.login - The user's GitHub login.
 * @param params.deps - Injected implementations (tests).
 */
export async function ensureFork(
  { owner, repo, login, deps = {} }: { owner: string; repo: string; login: string; deps?: { runImpl?: Function; remoteExistsImpl?: Function; borrowTokenImpl?: Function; forkRepoImpl?: Function } },
  { onProgress }: ProgressOptions = {},
): Promise<{ forkUrl: string; status: "exists" | "created" | "manual" }> {
  const {
    runImpl,
 ...
 */
export async function checkBaseUpdate({ dir }: { dir: string }, { onProgress }: ProgressOptions = {}): Promise<{ updateAvailable: boolean }> {
 ...
 * @throws {GitError} For other git failures.
 */
export async function applyBaseUpdate({ dir }: { dir: string }, { onProgress }: ProgressOptions = {}): Promise<{ merged: true }> {
  await assertBaseRepo(dir);
 ...
 */
async function assertBaseRepo(dir: string) {
  if (!(await isRepo(dir))) {
 ...
 */
export function resolveCampaignSource(source: string): { campaignId: string; branch: string } {
 ...
 */
export function classifyIncludeSource(source: string): "campaign" | "mod" {
 ...
 */
export function remoteNameForRepoUrl(repoUrl: string): string {
 ...
 */
export function resolveModSource(source: string, registry: Registry): { modId: string; entry: RegistryEntry } {
  if (typeof source !== "string" || !source.trim()) {
    throw new ValidationError("Include source must be a non-empty string.");
  }
 ...
 * Pure helper - exported for tests.
 */
export function upsertIncludedMod(existing: IncludedMod[] | undefined, entry: IncludedMod): IncludedMod[] {
  const list = Array.isArray(existing) ? [...existing] : [];
  const idx = list.findIndex((e) => e && e.id === entry.id);
  if (idx >= 0) {
    list[idx] = entry;
 ...
 * @param params.registry - Pre-fetched browse-tier registry. Fetched
 ...
 */
export async function includeMod({ dir, source, registry }: { dir: string; source: string; registry?: Registry }, { onProgress }: ProgressOptions = {}): Promise<{ modId: string; includedEntry: IncludedMod; commitHash: string; alreadyUpToDate: boolean }> {
  if (!(await isRepo(dir))) {
    throw new NotARepoError(dir);
  }
  ...
  const includedEntry: IncludedMod = {
    id: regEntry.id,
    name: regEntry.name,
 ...
 *   registry rollback).
 *
 * @param params.dir - Mod directory (must be a git repo).
 * @param params.registry - Pre-fetched browse-tier registry. Fetched
 ...
 * @throws {ManifestError} If the manifest is missing or invalid.
 */
export async function checkIncludedModsUpdates({ dir, registry }: { dir: string; registry?: Registry }, { onProgress }: ProgressOptions = {}): Promise<{ updates: IncludedModUpdate[]; registry: Registry | null }> {
  if (!(await isRepo(dir))) {
    throw new NotARepoError(dir);
  }
  ...
  const updates: IncludedModUpdate[] = [];
  for (const entry of entries) {
 ...
 */
function substituteScaffoldPath(relPath: string, modName: string): string {
 ...
 */
export function computeMissingScaffoldProduct(branch: string, manifest: RawManifest, catalog: ReadonlyArray<{ branch: string; product?: string }> = KNOWN_SCAFFOLDS): string | null {
...
export async function includeScaffold({ dir, source, scaffoldRepoUrl = SCAFFOLD_REPO_URL }: { dir: string; source: string; scaffoldRepoUrl?: string }, { onProgress }: ProgressOptions = {}): Promise<{ branch: string; scaffoldCommitHash: string; filesAdded: number; filesSkipped: number }> {
  ...
  let stamped: Array<{ src: string; dest: string }> = [];
