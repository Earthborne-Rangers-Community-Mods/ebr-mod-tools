...
type RunImpl = (command: string, args: string[], opts?: object) => Promise<RunResult>;
 ...
 */
export function normalizeGithubUrl(url: string | null): string | null {
  if (!url) return null;
 ...
 * @param token - GitHub token borrowed in memory for a single request.
 */
 ...
 * @param token - Token borrowed in memory for this single request.
 * @param options.owner - Upstream repo owner.
 * @param options.repo - Upstream repo name.
 */
export async function forkRepo(token: string, { owner, repo }: { owner: string; repo: string }): Promise<{ owner: string; repo: string; cloneUrl: string }> {
  try {
 ...
 *
 * @param options.input - Text written to stdin then closed.
 */
export function runCommand(command: string, args: string[], { input }: { input?: string } = {}): Promise<RunResult> {
 ...
 */
export function parseCredentialFill(stdout: string): Record<string, string> {
 ...
 *
 * @param options.interactive - When false, never prompt; return a cached credential or null.
 */
export async function borrowCredentialToken({ host = "github.com", interactive = true, runImpl = runCommand }: { host?: string; interactive?: boolean; runImpl?: RunImpl } = {}): Promise<string | null> {
 ...
 *
 * @returns true if `git credential reject` exited cleanly.
 */
export async function clearCredential({ host = "github.com", runImpl = runCommand }: { host?: string; runImpl?: RunImpl } = {}): Promise<boolean> {
  try {
