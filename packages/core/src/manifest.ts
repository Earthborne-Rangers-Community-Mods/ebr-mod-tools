...
import type { Manifest, RawManifest, ModValues, ProgressOptions, ManifestChange } from "./types.js";
 ...
 */
export function validateNonEmpty(val: any): true | string {
 ...
 */
export function validateName(val: string): true | string {
 ...
 */
export function validateIcon(val: any): true | string {
  if (typeof val !== "string" || !val.trim()) return "Cannot be empty.";
 ...
 */
export function validateId(val: string): true | string {
 ...
 */
export function validateVersion(val: string): true | string {
  if (typeof val !== "string" || !val) return "Cannot be empty.";
 ...
 */
export function validateLanguage(val: string): true | string {
  if (typeof val !== "string" || !val.trim()) return "Cannot be empty.";
  try {
 ...
 */
export function validateRepoUrl(val: string): true | string {
 ...
 * @param dir - Directory containing ebr-mod.json.
 * @returns Parsed, but not yet validated, manifest object.
 * @throws {ManifestError} If the file is missing or contains invalid JSON.
 */
export async function readManifest(dir: string): Promise<RawManifest> {
 ...
 * @param dir - Directory to write ebr-mod.json into.
 * @param manifest - Manifest object to serialize.
 */
export async function writeManifest(dir: string, manifest: object): Promise<void> {
  const filePath = join(dir, MANIFEST_FILENAME);
 ...
 */
export function validateManifest(input: unknown): ValidationErrorInfo[] {
 ...
 *
 * @param err - A validation error object from validateManifest().
 * @returns Human-readable error message.
 */
export function formatValidationError(err: ValidationErrorInfo): string {
  switch (err.code) {
 ...
 * @param errors - Array of validation error objects from validateManifest().
 ...
 *
 * @param manifest - A parsed, not-yet-validated manifest.
 * @returns The same object, narrowed to a validated manifest.
 * @throws {ManifestError} If validation fails (message lists every problem).
 */
export function assertValidManifest(manifest: RawManifest): Manifest {
 ...
 *
 * @returns -1 if a < b, 1 if a > b, 0 if equal, or `null` when
 ...
 *
 * @param input.type - Mod type (theme | campaign | one-day-mission | enhancement | expansion | collection).
 * @param input.campaigns - Selected campaign ids.
 * @param input.requiredProducts - Products the user marked required.
 * @param input.optionalProducts - Products already marked optional.
 * @returns The derived `optionalProducts` array.
 */
export function deriveOptionalProducts(
  { type, campaigns = [], requiredProducts = [], optionalProducts = [] }: {
    type: string;
    campaigns?: string[];
 ...
 *
 * @param manifest - The parsed manifest. `requiredProducts` and
 ...
 * @param missingProducts - Product ids to add.
 * @param bucket - Which list to add them to.
 * @returns The same manifest object (mutated).
 */
export function applyMissingProductFix(manifest: RawManifest, missingProducts: string[], bucket: "required" | "optional"): RawManifest {
 ...
 */
export function buildManifest(options: ModValues): Manifest {
 ...
 * @param options.repoUrl - GitHub URL to set (omit or null to skip).
 * @param callbacks.onProgress - Progress callback.
 */
export async function updateManifest(
  { dir, version, repoUrl }: { dir: string; version?: string; repoUrl?: string | null },
  { onProgress }: ProgressOptions = {},
): Promise<{ manifest: RawManifest; changes: ManifestChange[] }> {
