...
// --- fetchRemote / getLatestTag ---

describe("fetchRemote / getLatestTag", () => {
  let tmpDir;
  let bareDir;
  let sourceDir;

  beforeEach(async () => {
    // Create a bare "remote" with a tagged commit
    bareDir = await createBareRemote();
    ...
    await sourceGit.commit("base commit");
    await sourceGit.addTag("v1.0.0");
    const branch = (await sourceGit.branchLocal()).current;
    await sourceGit.push("origin", branch);
    await sourceGit.pushTags("origin");

    // Set up the working repo that will fetch from the bare remote
    tmpDir = await createTempDir();
    await initTestRepo(tmpDir);
    await commitFile(tmpDir, "readme.txt", "local content", "local initial commit");
    await addRemote(tmpDir, "base", bareDir);
  });
  ...
  it("fetches from a remote without throwing", async () => {
    await expect(fetchRemote(tmpDir, "base")).resolves.not.toThrow();
  });

  it("returns the latest tag after fetching", async () => {
    await fetchRemote(tmpDir, "base");
    const tag = await getLatestTag(tmpDir);
    expect(tag).toBe("v1.0.0");
  });

  it("returns the newest tag when multiple tags exist", async () => {
    // Push a second tag to the bare remote
    const sourceGit = simpleGit(sourceDir);
    await writeFile(join(sourceDir, "update.txt"), "updated content");
    await sourceGit.add("-A");
    await sourceGit.commit("update commit");
    await sourceGit.addTag("v1.1.0");
    const branch = (await sourceGit.branchLocal()).current;
    await sourceGit.push("origin", branch);
    await sourceGit.pushTags("origin");
...
describe("getLatestTag with no tags anywhere", () => {
  let tmpDir;
...
describe("getRemoteTags", () => {
  ...
  it("lists tags from a remote repo", async () => {
    ...
    // Set up working repo with source as remote
    await initTestRepo(tmpDir);
    await commitFile(tmpDir, "readme.txt", "local", "local commit");
    await addRemote(tmpDir, "base", sourceDir);

    const tags = await getRemoteTags(tmpDir, "base");
    expect(tags).toContain("v1.0.0");
    expect(tags).toContain("v1.1.0");
  });

  it("returns empty array for remote with no tags", async () => {
    sourceDir = await createTempDir();
    tempDirs.push(sourceDir);
    const sourceGit = simpleGit(sourceDir);
    await sourceGit.init();
